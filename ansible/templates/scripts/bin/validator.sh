...
identity_keypair={{ solana_public_key }}
identity_pubkey=$(solana-keygen pubkey "$identity_keypair")

ledger_dir={{ solana_ledger_location }}
args=(
...
if [[ -f {{ solana_identity_directory }}-vote-account.json ]]; then
  args+=(--vote-account {{ solana_identity_directory }}-vote-account.json)
else
  args+=(--no-voting)
fi

if [[ -f {{ solana_identity_directory }}-authorized-voter.json ]]; then
  args+=(--authorized-voter {{ solana_identity_directory }}-authorized-voter.json)
fi
...
if [[ -w {{ solana_accounts_location }} ]]; then
